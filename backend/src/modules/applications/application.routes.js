...
applicationRouter.get(
  ...
  async (req, res, next) => {
    try {
      const filters = incomingApplicationsQuerySchema.parse(req.query)
      const applications = await listIncomingApplications(req.user, filters)
      sendSuccess(res, { applications: applications.filter((application) => application.teamId === req.params.teamId) })
    } catch (error) {
      next(error)
    }
  },
...
applicationRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const application = await createApplication(req.user, input.teamId, input)
