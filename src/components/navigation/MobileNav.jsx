...
const navItems = [
  ...
  { label: 'Comps', ariaLabel: 'Competitions', to: routes.competitions, icon: Trophy },
  { label: 'Teams', to: routes.teams, icon: Users },
  { label: 'Apps', ariaLabel: 'Applications', to: routes.applications, icon: FileText },
  { label: 'Profile', to: routes.profile, icon: User },
]

export default function MobileNav() {
  return (
    <nav className="admin-mobile-nav lg:hidden" style={{ '--ui-mobile-nav-columns': navItems.length }}>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                [
                  ...
                  isActive
                    ? 'bg-[rgba(250,204,21,0.98)] text-[var(--admin-surface-low)] shadow-[0_16px_30px_rgba(250,204,21,0.16)]'
                    : 'text-[rgba(209,198,171,0.78)] hover:bg-[rgba(255,255,255,0.04)] hover:text-[var(--admin-gold-soft)]',
                ].join(' ')
              }
            >
              <Icon aria-hidden="true" size={20} />
              <span className="admin-ui-text text-[0.58rem] tracking-[0.14em]">{item.label}</span>
            </NavLink>
          )
        })}
      </div>
